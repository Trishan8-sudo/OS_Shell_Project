...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        mainLoop: while (true) {
            ...
            if (r.outputFile != null)
                ensureFile(r.outputFile, r.outputAppend);
            if (r.errorFile != null)
                ensureFile(r.errorFile, r.errorAppend);
            ...
            switch (command) {
                ...
                case "jobs":
                    ...
                    jobs.stream().sorted((a, b) -> Integer.compare(a.number, b.number))
                            .forEach(j -> writeOutput(formatJobLine(j, markers[0], markers[1]), r.outputFile,
                                    r.outputAppend));
        ...
        Redirection(List<String> raw) {
            for (int i = 0; i < raw.size(); i++) {
                ...
                if (t.matches("1?>{1,2}|2>{1,2}") && i + 1 < raw.size()) {
                    ...
                    if (err) {
                        errorFile = raw.get(i + 1);
                        errorAppend = t.endsWith(">>");
                    } else {
                        outputFile = raw.get(i + 1);
                        outputAppend = t.endsWith(">>");
                    }
                    i++;
                } else {
                    tokens.add(t);
    ...
    private static void markExitedJobs() {
        jobs.forEach(j -> {
            if (j.process != null && !j.process.isAlive())
                j.status = "Done";
        });
    }
    ...
    private static String formatJobLine(Job job, int cur, int prev) {
        ...
        if (job.status.equals("Done") && cmd.endsWith(" &"))
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (BUILTINS.contains(cmd)) {
                ...
                try {
                    ...
                    processes.add(p);
                    if (i != 0)
                        threads.add(pumpAsync(stageIn, p.getOutputStream()));
                    if (!isLast)
                        threads.add(pumpAsync(p.getInputStream(), stageOut));
                } catch (Exception e) {
    ...
    private static void executeBuiltin(List<String> tokens, java.io.PrintStream out, java.io.PrintStream err) {
        ...
        switch (command) {
            case "type":
                String rem = tokens.size() > 1 ? tokens.get(1) : "";
                if (BUILTINS.contains(rem))
                    out.println(rem + " is a shell builtin");
                else {
                    java.io.File exe = findExecutable(rem);
    ...
    private static void ensureFile(String path, boolean append) {
        try {
            ...
            if (!append || !file.exists())
                new java.io.FileWriter(file, append).close();
        } catch (Exception e) {
        }
    }
